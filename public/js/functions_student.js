...
document.addEventListener(
  ...
  function () {
    ...
    formUsuario.onsubmit = function (e) {
      ...
      let ajaxUrl = base_url + "/student/setStudent";
...
function fntEditStudent(id) {
  // Actualizar los titulos de la modal
  // Insertar -> Update
  qs("#titleModal").innerHTML = "Actualizar Usuario";
  document
  ...
  let ajaxUrl = base_url + "/student/getStudent/" + idStudent;
  ...
  request.onreadystatechange = function () {
    ...
    if (request.readyState == 4 && request.status == 200) {
      ...
      if (objData.status) {
        ...
        qs("#idStudent").value = objData.data.id;
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      let request = window.XMLHttpRequest
        ...
        : new ActiveXObject("Microsoft.XMLHTTP");
      // base_url = direccion del proyecto, se puede modificar en configs/config.php
      let ajaxUrl = base_url + "/student/delStudent/";
      let strData = "id=" + idStudent;
...
function openModal() {
  ...
  qs("#titleModal").innerHTML = "Add Student";
  qs("#userForm").reset();
  $("#modalFormStudent").modal("show");
}
