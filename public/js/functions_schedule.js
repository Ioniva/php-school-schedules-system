...
document.addEventListener(
  ...
  function () {
    ...
    const formSchedule = qs("#scheduleForm");
    // Se ejecuta esta funcion cuando pulsamos sobre le boton de Guardar del modal
    formSchedule.onsubmit = function (e) {
      // Mantener la modal abierta
      e.preventDefault();
      // Inicializar variables
      let time_start = qs("#time_start").value;
      let time_end = qs("#time_end").value;
      ...
      let ajaxUrl = base_url + "/schedule/setSchedule";
      ...
      request.onreadystatechange = function () {
        ...
        if (request.readyState == 4 && request.status == 200) {
          ...
          if (objData.status) {
            ...
            $("#modalFormSchedule").modal("hide");
            // Reseteamos los datos del formulario
            formSchedule.reset();
            // Mostramos mensaje de exito
            Swal.fire("Schedules", objData.msg, "success").then(() => {
...
function fntEditSchedule(id) {
  // Actualizar los titulos de la modal
  // Insertar -> Update
  qs("#titleModal").innerHTML = "Actualizar Horario";
  document
  ...
  let ajaxUrl = base_url + "/schedule/getSchedule/" + idSchedule;
  ...
  request.onreadystatechange = function () {
    ...
    if (request.readyState == 4 && request.status == 200) {
      ...
      if (objData.status) {
        ...
        qs("#idSchedule").value = objData.data.id_schedule;
        qs("#time_start").value = objData.data.time_start;
        qs("#time_end").value = objData.data.time_end;
        qs("#day").value = objData.data.day;
      }
      // Mostrar modal
      $("#modalFormSchedule").modal("show");
    }
  };
}

function fntDelSchedule(id) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      let request = window.XMLHttpRequest
        ...
        : new ActiveXObject("Microsoft.XMLHTTP");
      // base_url = direccion del proyecto, se puede modificar en configs/config.php
      let ajaxUrl = base_url + "/schedule/delSchedule/";
      let strData = "id=" + idSchedule;
...
function openModal() {
  ...
  qs("#titleModal").innerHTML = "Add Schedule";
